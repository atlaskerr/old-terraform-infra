...
resource "aws_security_group" "postgres_harbor" {
  name        = "postgres_harbor"
  description = "Postgres for Notary Security Group"
  vpc_id      = "${local.vpc_id}"

  tags {
    Name = "Postgresql Harbor"
  }
}

locals {
  sg_id       = "${aws_security_group.postgres_harbor.id}"
...
#resource "aws_security_group_rule" "pg_harbor_in" {
#  description       = "Allow Postgres port 5432 traffic from Harbor subnet"
#  type              = "ingress"
#  from_port         = "5432"
#  to_port           = "5432"
#  protocol          = "tcp"
#  cidr_blocks       = ["${local.harbor_cidr}"]
#  security_group_id = "${local.sg_id}"
#}
