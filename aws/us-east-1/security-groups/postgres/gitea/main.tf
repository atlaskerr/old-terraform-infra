...
resource "aws_security_group" "postgres_gitea" {
  name        = "postgres_gitea"
...
locals {
  sg_id      = "${aws_security_group.postgres_gitea.id}"
  vpn_cidr   = "${data.terraform_remote_state.cidr.admin_vpn_us_east_1b}"
  gitea_cidr = "${data.terraform_remote_state.cidr.git_us_east_1c}"
}
...
resource "aws_security_group_rule" "pg_gitea_in" {
  description       = "Allow Postgres port 5432 traffic from Gitea subnet"
  type              = "ingress"
  from_port         = "5432"
  to_port           = "5432"
  protocol          = "tcp"
  cidr_blocks       = ["${local.gitea_cidr}"]
  security_group_id = "${local.sg_id}"
}

resource "aws_security_group_rule" "pg_gitea_out" {
  description       = "Allow Postgres port 5432 traffic to Gitea subnet"
  type              = "egress"
  from_port         = "5432"
  to_port           = "5432"
  protocol          = "tcp"
